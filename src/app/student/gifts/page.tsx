  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-zinc-900">Sovg'alar do'koni</h1>
          <p className="text-sm text-zinc-500">To'plangan koinlaringiz evaziga olishingiz mumkin bo'lgan yutuqlar</p>
        </div>
        <div className="bg-amber-50 border border-amber-200/80 px-4 py-2 rounded-2xl flex items-center gap-2">
          <Coins className="w-4 h-4 text-amber-500" />
          <span className="text-xs font-semibold text-amber-800">Sizning balansingiz:</span>
          <span className="text-base font-extrabold text-amber-900 font-mono">{balance} koin</span>
        </div>
      </div>
              ...
              <div key={gift._id} className="bg-white border border-zinc-200/80 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold font-mono px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-600 flex items-center gap-1">
                      <Coins className="w-3.5 h-3.5" /> {gift.price_coin} koin
                    </span>
                    <span className="text-xs text-zinc-400">Qoldi: {gift.stock} dona</span>
                  </div>
                  <h3 className="text-base font-bold text-zinc-900">{gift.name}</h3>
                  <p className="text-xs text-zinc-500 mt-1">{gift.description || "Tavsif ko'rsatilmagan"}</p>
                </div>
