  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-zinc-900">Mening davomatim</h1>
          <p className="text-sm text-zinc-500">Darslarda ishtirok etish tarixi</p>
        </div>
        <div className="bg-white border border-zinc-200/80 px-4 py-2.5 rounded-2xl shadow-xs flex items-center gap-3">
          <span className="text-xs text-zinc-500 font-medium">Qatnashish foizi:</span>
          <span className="text-lg font-extrabold text-emerald-600 font-mono">{percentage}%</span>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-zinc-100">
              {records.map((r) => (
                <tr key={r._id} className="hover:bg-zinc-50/50">
                  <td className="px-6 py-4 font-medium text-zinc-900">{getRelationName(r.lesson_id)}</td>
